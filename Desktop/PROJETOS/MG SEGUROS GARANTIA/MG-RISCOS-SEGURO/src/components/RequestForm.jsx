...
function RequestForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      toast.loading('Enviando solicitação...', { id: 'submit' });

      // Upload dos arquivos
      let licitacaoURL = '';
      let cartaoCNPJTomadorURL = '';
      let cartaoCNPJAsseguradoURL = '';

      if (formData.licitacao) {
        licitacaoURL = await uploadFile(formData.licitacao);
      }
      if (formData.cartao_cnpj_tomador) {
        cartaoCNPJTomadorURL = await uploadFile(formData.cartao_cnpj_tomador);
      }
      if (formData.cartao_cnpj_assegurado) {
        cartaoCNPJAsseguradoURL = await uploadFile(formData.cartao_cnpj_assegurado);
      }

      // Montar dados para o Firestore
      const docData = {
        timestamp: serverTimestamp(),
        edital: formData.edital,
        licitacaoURL,
        tomador: {
          empresa: formData.empresa_tomador,
          cnpj: formData.cnpj_tomador,
          endereco: formData.endereco_tomador,
          cartaoCNPJURL: cartaoCNPJTomadorURL || null
        },
        assegurado: {
          empresa: formData.empresa_assegurado,
          cnpj: formData.cnpj_assegurado,
          endereco: formData.endereco_assegurado,
          cartaoCNPJURL: cartaoCNPJAsseguradoURL || null
        },
        status: 'novo',
        observacoes: null,
      ...
      toast.success('Solicitação enviada com sucesso!', { id: 'submit' });
      setFormData({
        edital: '',
        licitacao: null,
        empresa_tomador: '',
      ...
      toast.error('Erro ao enviar solicitação', { id: 'submit' });
      console.error(error);
    }
  };

  const inputClasses = "mt-4 block w-full h-12 py-3 px-4 rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary bg-gray-150";
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      onChange={e => setFormData({ ...formData, licitacao: e.target.files[0] })}
                    />
                  </div>
                </div>
              </div>
