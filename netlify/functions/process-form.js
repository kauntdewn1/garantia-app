...
// Função para salvar na planilha Google - COMPATÍVEL com versão 4.1.1
async function saveToSpreadsheet(data) {
  try {
    const doc = new GoogleSpreadsheet(spreadsheetConfig.spreadsheetId);
    
    // Na versão 4.1.1, precisamos configurar o cliente JWT
    const client = new doc.constructor.JWT({
      email: spreadsheetConfig.credentials.client_email,
      key: spreadsheetConfig.credentials.private_key,
